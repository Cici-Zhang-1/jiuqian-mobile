...
class Role_page_form extends MY_Controller {
    ...
    public function read() {
        ...
        if ($V > 0) {
            if (!!($Role = $this->role_model->is_exist($V))) {
                if (!!($Query = $this->role_page_form_model->select_by_role_v($Role['v']))) {
    ...
    public function edit(){
        ...
        if ($this->_do_form_validation()) {
            ...
            $this->role_page_form_model->delete_by_role_v($Post['role_v']);
            ...
            $this->role_page_form_model->insert_batch($Data);
