...
class Page_form extends MY_Controller {
    ...
    public function read() {
        ...
        $Data = array();
        if(!($Data = $this->page_form_model->select($this->_Search))){
            $this->Message = isset($GLOBALS['error'])?is_array($GLOBALS['error'])?implode(',', $GLOBALS['error']):$GLOBALS['error']:'读取信息失败';
            $this->Code = EXIT_ERROR;
        }
        $Data['query']['form_page_v'] = $this->_Search['v'];
    ...
    public function add() {
        if ($this->_do_form_validation()) {
            ...
            if(!!($Fid = $this->page_form_model->insert($Post))) {
                ...
                $this->role_page_form_model->insert(array('role_v' => SUPER_NO, 'page_form_v' => $Fid));
    ...
    public function remove(){
        ...
        if ($this->_do_form_validation()) {
            ...
            if ($this->page_form_model->delete($Where)) {
