...
class Order_product_door extends MY_Controller {
    ...
    public function __construct() {
        parent::__construct();
        log_message('debug', 'Controller order/Order_product_door __construct Start!');
    ...
    */
    ...
    public function read () {
        ...
        $Data = array();
        if (!empty($this->_Search['order_product_id'])) {
            if(!($Data = $this->order_product_door_model->select_one($this->_Search))) {
                $this->Message = isset($GLOBALS['error'])?is_array($GLOBALS['error'])?implode(',', $GLOBALS['error']):$GLOBALS['error']:'读取信息失败';
                $this->Code = EXIT_ERROR;
            }
        } else {
            $this->Message = '请选择需要查看结构的门板';
            $this->Code = EXIT_ERROR;
        }
    ...
    public function add() {
        if ($this->_do_form_validation()) {
            ...
            if(!!($NewId = $this->order_product_door_model->insert($Post))) {
    ...
    */
    public function edit() {
        if ($this->_do_form_validation()) {
            ...
            $Where = $Post['v'];
            unset($Post['v']);
            if(!!($this->order_product_door_model->update($Post, $Where))){
    ...
    public function remove() {
        ...
        if ($this->_do_form_validation()) {
            ...
            if ($this->order_product_door_model->delete($Where)) {
