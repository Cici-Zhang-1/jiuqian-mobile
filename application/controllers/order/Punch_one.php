 ...
 * 打孔
 */
class Punch_one extends MY_Controller{
    ...
    public function __construct(){
        parent::__construct();
        log_message('debug', 'Controller order/Punch_one __construct Start!');
        $this->load->model('order/punch_one_model');
    }
    ...
    public function read(){
        ...
        $Data = array();
        if(!($Data = $this->punch_one_model->select($this->_Search))){
    ...
    private function _edit_order_product_classify () {
        if (!empty($this->_Classify)) {
            ...
            if (!!($Query = $this->order_product_classify_model->is_status_and_brothers($this->_Classify, WP_PUNCH, P_PUNCH_ONE))) {
    ...
    private function _edit_order_product_board () {
        if (!empty($this->_Board)) {
            ...
            if (!!($Query = $this->order_product_board_model->is_status_and_brothers($this->_Board, WP_PUNCH, P_PUNCH_ONE))) {
                $GLOBALS['workflow_msg'] = '';
                foreach ($Query as $Key => $Value) {
                    $GLOBALS['workflow_msg'] .= $Value['board'];
                    $Query[$Key] = $Value['v'];
