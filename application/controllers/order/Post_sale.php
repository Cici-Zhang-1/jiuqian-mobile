    ...
    public function read () {
        ...
        if (empty($this->_Search['order_product_id'])) {
            ...
            if (!($Data['order_info'] = $this->order_product_model->is_order_post_salable($this->_Search['order_product_id']))) {
                $this->Code = EXIT_ERROR;
                $this->Message = '订单当前状态不可受后!';
            }
        }
        if ($this->Code == EXIT_SUCCESS) {
            $OrderProducts = array();
            if (!!($Query = $this->order_product_model->select_post_sale($Data['order_info']['order_id']))) {
        ...
        if (!!($OrderProduct = $this->_is_post_salable())) {
            ...
            if (!!($P = $this->p->initialize($OrderProduct['code']))) {
                if (!($P->edit($Save, $OrderProduct))) {
                    $this->Code = EXIT_ERROR;
                    $this->Message = isset($GLOBALS['error'])?is_array($GLOBALS['error'])?implode(',', $GLOBALS['error']):$GLOBALS['error']:'订单产品售后出错!';
                } else {
                    $this->Message = '售后成功处理!';
                }
            } else {
                $this->Code = EXIT_ERROR;
                $this->Message = '您售后的订单产品类型不存在!';
            }
        } else {
            $this->Code = EXIT_ERROR;
            $this->Message = isset($GLOBALS['error'])?is_array($GLOBALS['error'])?implode(',', $GLOBALS['error']):$GLOBALS['error']:'订单产品不可售后!';
        }
        $this->_ajax_return();
