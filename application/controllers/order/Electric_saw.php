 ...
 * 电子锯
 */
class Electric_saw extends CWDMS_Controller{
    ...
    public function __construct(){
        ...
        log_message('debug', 'Controller Order/Electric_saw Start!');
    }
    ...
    public function edit(){
        ...
        if($this->form_validation->run($Item)){
            ...
            if(!!($Batch = $this->order_product_classify_model->select_batch($Selected))){
                $this->load->library('workflow/workflow');
                foreach ($Batch as $key => $value){
                    $this->workflow->initialize('order_product_classify', $value['opcid']);
                    $this->workflow->electric_saw();
                }
            }else{
                $this->Failue .= isset($GLOBALS['error'])?is_array($GLOBALS['error'])?implode(',', $GLOBALS['error']):$GLOBALS['error']:'没有需要确认下料的批次';
            }
        }else{
