...
?>
            <div class="col-md-12" id="dismantleP">
				<form class="form-horizontal" method="post" id="dismantlePForm">
				    <div class="form-group col-md-12">
						<p class="form-control-static" id="dismantlePFormError"></p>
					</div>
					...
					<div class="col-md-6">
    					<table	class="table-center table-form table table-bordered table-striped table-condensed col-md-6" id="dismantlePTable">
    						<thead>
    							<tr>
    								<th class="td-xs" >#</th>
    								<th >名称</th>
					...
					<div class="col-md-6">
    					<table class="table table-bordered table-striped table-condensed" id="dismantleFittingTable">
    						<thead>
    							<tr>
    							    <th>#</th>
    								<th>名称<input id="dismantleFittingTableSearch" class="form-control input-sm"/></th>
    								<th>单位</th>
    								<th class="hide">id</th>
    							</tr>
    						</thead>
			...
			<script>
				(function($){
					var clear_peijian_data = function(){
						$('#dismantlePTable tbody tr:gt(0)').remove();
						$('#dismantlePTable tbody tr:eq(0) input[name="name"]').val('');
						};
					var load_peijian_data = function(Opid){
						$.ajax({
							async: true,
							url: '<?php echo site_url('order/dismantle_p/read');?>',
							data: {id: Opid},
							dataType: 'json',
							type: 'GET',
					...
					$.when(A1, A2).done(function(V1, V2){
						...
						for(i in Fitting){
							if(undefined == FormatFitting[Fitting[i]['type']]){
								FormatFitting[Fitting[i]['type']] = '<tr><td></td><td ><a href="javascript:void(0)" data-placement="top" data-toggle="tooltip" title="'+Fitting[i]['unit_price']+'">'+Fitting[i]['name']+'</a></td><td>'+Fitting[i]['unit']+'</td><td class="hide">'+Fitting[i]['fid']+'</td></tr>';
							}else{
								FormatFitting[Fitting[i]['type']] += '<tr><td></td><td ><a href="javascript:void(0)" data-placement="top" data-toggle="tooltip" title="'+Fitting[i]['unit_price']+'">'+Fitting[i]['name']+'</a></td><td>'+Fitting[i]['unit']+'</td><td class="hide">'+Fitting[i]['fid']+'</td></tr>';
							}
						}
						...
						$('#dismantleFittingTable tbody').html(FormatFittingType.reverse().join(''));

						$('#dismantleFittingTable a[data-toggle="tooltip"]').tooltip();
						$('#dismantleFittingTable').find('tbody tr').each(function(i, v){
							$(this).on('dblclick', function(e){
								var $This = $(this), $Last = $('#dismantlePTable tbody').children('tr:last'), $Clone = $Last.clone(true);
						...
						$('#dismantleFittingTableSearch').keyup(function(){
							$('#dismantleFittingTable').find('tbody tr').hide().filter(":contains('"+( $(this).val() )+"')").show();
						});
						
						if(0 != $('#dismantlePForm select[name="opid"]').val()){
							load_peijian_data($('#dismantlePForm select[name="opid"]').val());
						}
					});
					$('#dismantlePTable').table_form({DeleteLine:true});
					
					$('#dismantlePForm select[name="parent"]').on('change', function(e){
						var Parent = $(this).val();
						$('#dismantlePForm select[name="opid"] option').show().not('[data-parent="'+Parent+'"]').hide();
					});
					$('#dismantlePForm select[name="opid"]').on('change', function(e){
						if(0 != $(this).val()){
    						...
    						$('#dismantlePForm input[name="product"]').val($Selected.data('product'));
    						$('#dismantlePForm input[name="remarks"]').val($Selected.data('remarks'));
