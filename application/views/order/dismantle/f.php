...
?>
            <div class="col-md-12" id="dismantleF">
				<form class="form-horizontal" method="post" id="dismantleFForm">
					<div class="form-group col-md-12">
						<p class="form-control-static" id="dismantleFFormError"></p>
					</div>
					...
					<div class="col-md-6">
    					<table	class="table-center table-form table table-bordered table-striped table-condensed col-md-6" id="dismantleFTable">
    						<thead>
    							<tr>
    								<th class="td-xs" >#</th>
    								<th >名称</th>
					...
					<div class="col-md-6">
    					<table class="table table-bordered table-striped table-condensed" id="dismantleServerTable">
    						<thead>
    							<tr>
    								<th class="td-xs" >#</th>
    								<th >名称<input id="dismantleServerTableSearch" class="form-control input-sm"/></th>
    								<th >单位</th>
    								<th class="hide">sid</th>
    							</tr>
    						</thead>
			...
			<script>
				(function($){
					var clear_server_data = function(){
						$('#dismantleFTable tbody tr:gt(0)').remove();
						$('#dismantleFTable tbody tr:eq(0) input[name="name"]').val('');
						};
					var load_server_data = function(Opid){
						$.ajax({
							async: true,
							url: '<?php echo site_url('order/dismantle_f/read');?>',
							data: {id: Opid},
							dataType: 'json',
							type: 'GET',
					...
					$.when(A1, A2).done(function(V1, V2){
						...
						for(i in Server){
							if(undefined == FormatServer[Server[i]['type']]){
								FormatServer[Server[i]['type']] = '<tr><td></td><td >'+Server[i]['name']+'</td><td>'+Server[i]['unit']+'</td><td class="hide">'+Server[i]['sid']+'</td></tr>';
							}else{
								FormatServer[Server[i]['type']] += '<tr><td></td><td >'+Server[i]['name']+'</td><td>'+Server[i]['unit']+'</td><td class="hide">'+Server[i]['sid']+'</td></tr>';
							}
						}
						...
						$('#dismantleServerTable tbody').html(FormatServerType.reverse().join(''));
						$('#dismantleServerTable').find('tbody tr').each(function(i, v){
							$(this).on('dblclick', function(e){
								var $This = $(this), $Last = $('#dismantleFTable tbody').children('tr:last'), $Clone = $Last.clone(true);
						...
						$('#dismantleServerTableSearch').keyup(function(){
							$('#dismantleServerTable').find('tbody tr:not(.success)').hide().filter(":contains('"+( $(this).val() )+"')").show();
						});
						
						if(0 != $('#dismantleFForm select[name="opid"]').val()){
							load_server_data($('#dismantleFForm select[name="opid"]').val());
						}
					});

					$('#dismantleFTable').table_form({DeleteLine:true});
					
					$('#dismantleFForm select[name="parent"]').on('change', function(e){
						var Parent = $(this).val();
						$('#dismantleFForm select[name="opid"] option').show().not('[data-parent="'+Parent+'"]').hide();
					});
					$('#dismantleFForm select[name="opid"]').on('change', function(e){
						if(0 != $(this).val()){
    						...
    						$('#dismantleFForm input[name="product"]').val($Selected.data('product'));
							$('#dismantleFForm input[name="remarks"]').val($Selected.data('remarks'));
