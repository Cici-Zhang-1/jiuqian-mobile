...
?>
            <div class="col-md-12" id="dismantleG">
				<form class="form-horizontal" method="post" id="dismantleGForm">
					<div class="form-group col-md-12">
						<p class="form-control-static" id="dismantleGFormError"></p>
					</div>
					...
					<div class="col-md-8">
    					<table	class="table-center table-form table table-bordered table-striped table-condensed col-md-6" id="dismantleGTable">
    						<thead>
    							<tr>
    								<th class="td-xs" >#</th>
    								<th class="td-sm" >名称</th>
    								<th >规格</th>
    						...
    						<tbody>
    							<tr>
    								...
    								<td><input class="form-control input-sm" name="spec" type="text" placeholder="高x宽x块"/></td>
    								<td><input class="form-control input-sm" name="amount" type="text" placeholder="块数或面积"/></td>
					...
					<div class="col-md-4">
    					<table class="table table-bordered table-striped table-condensed" id="dismantleOtherTable">
    						<thead>
    							<tr>
    								<th class="td-xs" >#</th>
    								<th >名称<input id="dismantleOtherTableSearch" class="form-control input-sm"/></th>
    								<th >规格</th>
    								<th >单位</th>
    								<th class="hide">oid</th>
    							</tr>
			...
			<script>
				(function($){
					var clear_other_data = function(){
						$('#dismantleGTable tbody tr:gt(0)').remove();
						$('#dismantleGTable tbody tr:eq(0) input[name="name"]').val('');
					};
					var load_other_data = function(Opid){
						$.ajax({
							async: true,
							url: '<?php echo site_url('order/dismantle_g/read');?>',
							data: {id: Opid},
							dataType: 'json',
							type: 'GET',
					...
					$.when(A1, A2).done(function(V1, V2){
						...
						for(i in Other){
							if(undefined == FormatOther[Other[i]['type']]){
								FormatOther[Other[i]['type']] = '<tr><td></td><td >'+Other[i]['name']+'</td><td>'+Other[i]['spec']+'</td><td>'+Other[i]['unit']+'</td><td class="hide">'+Other[i]['oid']+'</td></tr>';
							}else{
								FormatOther[Other[i]['type']] += '<tr><td></td><td >'+Other[i]['name']+'</td><td>'+Other[i]['spec']+'</td><td>'+Other[i]['unit']+'</td><td class="hide">'+Other[i]['oid']+'</td></tr>';
							}
						}
						...
						$('#dismantleOtherTable tbody').html(FormatOtherType.reverse().join(''));
						$('#dismantleOtherTable').find('tbody tr').each(function(i, v){
							$(this).on('dblclick', function(e){
								var $This = $(this), $Last = $('#dismantleGTable tbody').children('tr:last'), $Clone = $Last.clone(true);
						...
						$('#dismantleOtherTableSearch').keyup(function(){
							$('#dismantleOtherTable').find('tbody tr:not(.success)').hide().filter(":contains('"+( $(this).val() )+"')").show();
						});
						
						if(0 != $('#dismantleGForm select[name="opid"]').val()){
							load_other_data($('#dismantleGForm select[name="opid"]').val());
						}
					});
					$('#dismantleGTable').table_form({DeleteLine:true});
					
					$('#dismantleGForm select[name="parent"]').on('change', function(e){
						var Parent = $(this).val();
						$('#dismantleGForm select[name="opid"] option').show().not('[data-parent="'+Parent+'"]').hide();
					});
					$('#dismantleGForm select[name="opid"]').on('change', function(e){
						if(0 != $(this).val()){
    						...
    						$('#dismantleGForm input[name="product"]').val($Selected.data('product'));
    						$('#dismantleGForm input[name="remarks"]').val($Selected.data('remarks'));
