...
class Pack_model extends MY_Model {
    ...
    public function __construct() {
        ...
        log_message('debug', 'Model order/Pack_model Start!');
    }
    ...
    public function select ($Search) {
        ...
        if (!($Return = $this->cache->get($Cache))) {
            ...
            if(!empty($Search['pn'])){
                ...
                if ($Search['status'] == WP_PACKED) {
                    $Sql = $Sql . ' ORDER BY pack_datetime desc LIMIT ' . ($Search['p']-1)*$Search['pagesize'] . ', ' . $Search['pagesize'];
                } else {
    ...
    private function _select_order_product_classify ($Search) {
        ...
        if ($Search['status'] == WP_PACK) {
            ...
            $this->HostDb->where('opc_pack > ', ZERO);
            $this->HostDb->where('opc_pack_datetime is not null');
            if (!empty($Search['start_date'])) {
                $this->HostDb->where('opc_pack_datetime >= ', $Search['start_date']);
            }
            if (!empty($Search['end_date'])) {
                $this->HostDb->where('opc_pack_datetime <= ', $Search['end_date']);
            }
        }
    ...
    private function _select_order_product_board ($Search) {
        ...
        if ($Search['status'] == WP_PACK) {
            ...
            $this->HostDb->where('opb_pack > ', ZERO);
            $this->HostDb->where('opb_pack_datetime is not null');
            if (!empty($Search['start_date'])) {
                $this->HostDb->where('opb_pack_datetime >= ', $Search['start_date']);
            }
            if (!empty($Search['end_date'])) {
                $this->HostDb->where('opb_pack_datetime <= ', $Search['end_date']);
            }
        }
