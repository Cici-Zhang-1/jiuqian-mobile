...
class Print_list_model extends MY_Model {
    ...
    private function _select_yes ($Cache, $Search) {
        ...
        if(!empty($Search['pn'])){
            ...
            if (!empty($Search['keyword'])) {
                $Sql .= " WHERE op_num like '%" . $Search['keyword'] . "%' && opc_print > " . ZERO . ' GROUP BY op_id, opc_classify_id';
            } else {
                $Sql .= " WHERE opc_print > " . ZERO . ' GROUP BY op_id, opc_classify_id';
            }
            ...
            if (!empty($Search['keyword'])) {
                $Sql .= " WHERE op_num like '%" . $Search['keyword'] . "%' && opb_print > " . ZERO . ' GROUP BY op_id';
            } else {
                $Sql .= " WHERE opb_print > " . ZERO . ' GROUP BY op_id';
            }

            $Sql .= " ORDER BY print_datetime desc LIMIT " . ($Search['p']-1)*$Search['pagesize'] . ', ' . $Search['pagesize'];
    ...
    private function _page_yes ($Search) {
        $Sql = "SELECT opc_id AS v FROM j_order_product_classify LEFT JOIN j_order_product ON op_id = opc_order_product_id";
        if (!empty($Search['keyword'])) {
            $Sql .= " WHERE op_num like '%" . $Search['keyword'] . "%' && opc_print > " . ZERO . ' GROUP BY opc_order_product_id, opc_classify_id';
        } else {
            $Sql .= " WHERE opc_print > " . ZERO . ' GROUP BY opc_order_product_id, opc_classify_id';
        }
        $Sql .= " UNION ALL SELECT opb_id AS v FROM j_order_product_board LEFT JOIN j_order_product ON op_id = opb_order_product_id";
        if (!empty($Search['keyword'])) {
            $Sql .= " WHERE op_num like '%" . $Search['keyword'] . "%' && opb_print > " . ZERO . ' GROUP BY opb_order_product_id';
        } else {
            $Sql .= " WHERE opb_print > " . ZERO . ' GROUP BY opb_order_product_id';
        }
