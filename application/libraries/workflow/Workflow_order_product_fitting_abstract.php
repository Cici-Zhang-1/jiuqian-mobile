 ...
 * @des
 */
abstract class Workflow_order_product_fitting_abstract{
    ...
    public function set_workflow(Workflow_order_product_fitting $Workflow){
    ...
    protected function _get_order_product_id(){
        ...
        if(!!($Ids = $this->_CI->order_product_fitting_model->select_order_product_id($this->_Source_ids))){
            return $Ids;
        }
        return false;
    }
    ...
    protected function _workflow_next () {
        $this->_Source_ids = $this->_Workflow->get_source_ids();
        if(!!($WorkflowNext = $this->_CI->order_product_fitting_model->select_workflow_next($this->_Source_ids))){
            foreach ($WorkflowNext as $Key => $Value) {
                $this->_Workflow->initialize($Value['v']); // 初始化v
                $this->_Workflow->edit_current_workflow(Workflow_order_product_fitting::$AllWorkflow[$Value['workflow_procedure_name']], array('procedure' => $Value['procedure']));
