...
class Workflow_mrp {
    ...
    private $_Workflow;
    private $_V;
    private $_Type = 'Mrp'; // mrp
    private $_Model = 'mrp_model';
    private $_Source_id;
    private $_Source_ids;
    private $_Dir;
    private $_CI;
    ...
    public function __construct() {
        ...
        log_message('debug', 'Library Workflow/Workflow_mrp __construct Start');
        $this->_Dir = dirname(__FILE__);
        $this->_CI->load->model('workflow/workflow_mrp_model');
        $this->_CI->load->model('workflow/workflow_mrp_msg_model');
    }
    ...
    public function initialize($Source_id, $Data = array()) {
        ...
        if(is_null(self::$AllWorkflow)){
            if(!!($Query = $this->_CI->workflow_mrp_model->select(array('pagesize' => ALL_PAGESIZE, 'p' => ONE)))) {
                $Query = $Query['content'];
                foreach ($Query as $key => $value) {
                    self::$AllWorkflow[$value['name']] = $value;
                }
            } else {
                $this->_Failue = '获取MRP工作流失败!';
                return false;
            }
        }
    ...
    private function _set_context($Workflow) {
        $this->_V = $Workflow['v'];
        require_once $this->_Dir.'/Workflow_mrp_abstract.php';
        $File = $this->_Dir . '/workflow_mrp/' . $Workflow['file'].'.php';
        if (file_exists($File)) {
            require_once $File;
            log_message('debug', 'Library Workflow/workflow_mrp _set_content on File = ' . $File.' And Source_id' . $this->_Source_id);
            $Class = '\\Wm\\' . $Workflow['file'];
        ...
        if(is_array($this->_Source_ids)){
            ...
            foreach ($this->_Source_ids as $value){
                $Set[] = array(
                    ...
                    'workflow_mrp_id' => $this->_V
                );
            }
            $this->_CI->workflow_mrp_msg_model->insert_batch($Set);
        }else{
            $Set = array(
                'mrp_id' => $this->_Source_id,
                'msg' => $Msg . $GLOBALS['workflow_msg'],
                'workflow_mrp_id' => $this->_V
            );
