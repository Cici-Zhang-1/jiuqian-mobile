 ...
 * Time: 9:26
 */
require_once 'Workflow_order_product_other_abstract.php';

class Workflow_order_product_other {
    static $AllWorkflow = NULL;
    private $_Workflow;
    private $_V;
    private $_Type = 'Order_product_other'; // order_product_other
    private $_Model = 'order_product_other_model';
    private $_Source_id;
    private $_Source_ids;
    private $_Dir;
    private $_CI;
    ...
    public function __construct() {
        ...
        log_message('debug', 'Library Workflow/Workflow_order_product_other __construct Start');
        $this->_Dir = dirname(__FILE__);
        $this->_CI->load->model('workflow/workflow_procedure_model');
        $this->_CI->load->model('workflow/workflow_order_product_other_msg_model');
    }
    ...
    private function _set_context($Workflow) {
        ...
        $File = $this->_Dir . '/workflow_order_product_other/' . $Workflow['file'].'.php';
        if (file_exists($File)) {
            require_once $File;
            log_message('debug', 'Library Workflow/workflow_order_product_other _set_content on File = '.$File.' And Source_id'.$this->_Source_id);
            $Class = '\\Wopo\\' . $Workflow['file'];
    ...
    public function store_message($Msg) {
        ...
        if(is_array($this->_Source_ids)){
            ...
            foreach ($this->_Source_ids as $value){
                $Set[] = array(
                    'order_product_other_id' => $value,
                    'msg' => $Msg . $GLOBALS['workflow_msg'],
                    'workflow_order_product_other_id' => $this->_V
                );
            }
            $this->_CI->workflow_order_product_other_msg_model->insert_batch($Set);
        }else{
            $Set = array(
                'order_product_other_id' => $this->_Source_id,
                'msg' => $Msg . $GLOBALS['workflow_msg'],
                'workflow_order_product_other_id' => $this->_V
            );
            $this->_CI->workflow_order_product_other_msg_model->insert($Set);
        }
        return true;
    }
