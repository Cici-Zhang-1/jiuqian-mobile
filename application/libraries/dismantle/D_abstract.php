...
abstract class  D_abstract {
    ...
    protected function _is_valid_board ($BoardPlate) {
        ...
        if (isset(self::$_Board)) {
            if (isset(self::$_Board[$BoardPlate])) {
                ...
                $this->_Failue = $BoardPlate . '不在系统中, 请先登记板材!';
            }
        } else {
            $this->_Failue = '系统中没有板块信息';
        }
        return false;
    }
     ...
     * @param unknown $Opid
     */
    protected function _edit_order_product($Set, $OrderProductId){
        $Set = gh_escape($Set);
        $this->_CI->order_product_model->update($Set, $OrderProductId);
    }
