<?php $this->load->view('layout/header'); ?>
	...
	<?php echo form_open('{{ plural }}/create', 'class=""'); ?>
...
{% endif %}
			...
			<div class="">
{% if column.field in foreignKeys|keys %}
				<?php echo form_dropdown('{{ column.field }}', ${{ foreignKeys[column.field] }}, set_value('{{ column.field }}'), 'class="{{ bootstrap
...
{% else %}
				<?php echo form_input('{{ column.field }}', set_value('{{ column.field }}'), 'class="{{ bootstrap.formControl }}" {{ column.isNull ? '' : 'required' }}'); ?>
{% endif %}
